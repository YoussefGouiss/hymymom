    ...
    try {
      ...
      if (!response.ok) {
        setError({
          code: 'ERROR',
          title: 'Error',
          message: resData.message || 'Failed to send code',
          type: 'error'
        });
        setIsLoading(false);
        return;
      ...
      <div className="min-h-screen flex items-center justify-center bg-cloud-white dark:bg-[#030712] text-on-surface dark:text-slate-100 font-body relative overflow-hidden px-4 py-12 transition-colors duration-300">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="glass-card vanta-glow p-8 md:p-10 shadow-2xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 text-center">
            ...
            <Link href="/login" className="inline-flex items-center gap-2 px-6 py-3 bg-baby-blue dark:bg-sky-500 text-white dark:text-slate-950 rounded-2xl font-bold hover:opacity-90 transition-all">
              Go to Login
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <div className="flex items-center justify-center gap-2 mt-3">
            ...
            <p className="text-on-surface-variant dark:text-slate-400 font-medium tracking-wide text-sm uppercase">
              {step === 1 ? 'Reset Your Key' : 'Enter New Key'}
            </p>
            <div className="h-px w-8 bg-baby-blue/20 dark:bg-sky-500/30"></div>
          </div>
        </div>
        ...
        <div className="glass-card vanta-glow p-8 md:p-10 shadow-2xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5">
          ...
          {step === 1 ? (
            <form onSubmit={handleSendCode}>
              ...
              >
                {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : <>Send Reset Code <ArrowRight className="w-5 h-5" /></>}
              </button>
            </form>
          ) : (
            <form onSubmit={handleResetPassword}>
              <div className="mb-6" onPaste={handlePaste}>
                ...
                <button type="button" onClick={() => setStep(1)} className="block mx-auto mt-3 text-sm text-baby-blue dark:text-sky-400 hover:underline">
                  Change email
                </button>
              </div>

              <div className="mb-4">
                <label className="block text-xs font-bold text-baby-blue dark:text-sky-400 uppercase tracking-widest mb-2 px-1">New Password</label>
                <div className="relative">
              ...
              <div className="mb-6">
                <label className="block text-xs font-bold text-baby-blue dark:text-sky-400 uppercase tracking-widest mb-2 px-1">Confirm Password</label>
                <div className="relative">
                  <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                  <input
                    type="password"
              ...
              <button
                ...
                className="w-full py-4.5 bg-baby-blue dark:bg-sky-500 text-white dark:text-slate-950 rounded-2xl font-bold text-lg hover:opacity-90 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 shadow-xl shadow-baby-blue/20 dark:shadow-sky-500/20 disabled:opacity-50"
              >
                {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : <>Reset Password <ArrowRight className="w-5 h-5" /></>}
              </button>
            </form>
          )}
